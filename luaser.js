...
🥳 WELCOME 🫂`
				...
				teks = `*_「 ᴘʀᴏᴍᴏᴛᴇ ᴅᴇᴛᴇᴄᴛᴇᴅ 」_*\n@${num.split('@')[0]} 𝙴𝚗𝚝𝚎 ᴍᴏɴᴜsᴇ ᴄᴏɴɢʀᴀᴛs ʏᴏᴜ ᴀʀᴇ ɴᴏᴡ ᴀᴅᴍɪɴ`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				...
				teks = `*_「 ᴅᴇᴍᴏᴛᴇ ᴅᴇᴛᴇᴄᴛᴇᴅ 」_*\n@${num.split('@')[0]} ᴍᴇʜ ᴍᴏᴏɴᴊɪ 𝙷𝙾𝙾𝙼..ʏᴏᴜ ᴀʀᴇ ᴅᴇɴᴏᴛᴇᴅ ᴀs ᴍᴇᴍʙᴇʀ`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				...
				teks = `ɢᴏᴏᴅ ʙʏᴇ @${num.split('@')[0]}👋\nʀᴇsᴛ ɪɴ ᴘᴇᴀᴄᴇ...𝙽𝙴𝙴 𝚂𝙴𝚃𝙷𝙾 𝙼𝙾𝙽𝚄 ⚰💀🕊️`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*👻 ᴡᴀɪᴛ 𝙲𝙷𝚈 𝙼𝙾𝙽𝚄 ᴛᴏ ᴘʀᴏᴄᴇ𝚜𝚜ᴇᴅ 𝚉𝙾𝙾𝙽... ...*',
				success: '*sᴜᴄᴄᴇss...*',
				error: {
...
denz.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "type #menu sis", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
		...
		reply(`*_「 ʙᴀᴅᴡᴏʀᴅ ᴅᴇᴛᴇᴄᴛᴇᴅ 」_*\nsᴏʀʀʏ *${pushname2}* ʏᴏᴜ ᴀʀᴇ ᴛᴀʟᴋɪɴɢ ᴅɪʀᴛʏ!, ʏᴏᴜ ᴀʀᴇ ɪᴍᴍᴇᴅɪᴀᴛᴇʟʏ ɪɴ ᴛʜᴇ ᴋɪᴄᴋ ᴏғ ᴛʜᴇ ɢʀᴏᴜᴘ *${groupMetadata.subject}*`)
			...
			dmenu = `╓───「 *_𝗝𝐎𝐂𝐊𝐄𝐑 𝐁𝐎𝐓 𝐔𝐘𝐈𝐑_* 」
║𝐻𝐴𝐼 𝐼'𝑀 𝐵𝑂𝑇  🕊️
╙───々
╓───「 *_ʙᴏᴛ ɪɴғᴏ_* 」
║🧧 ᴄʀᴇᴀᴛᴏʀ: 𝟒 𝟒 𝟒
║🧾 ʀᴇᴄᴏᴅᴇ ʙʏ: 𝙹𝙾𝙲𝙺𝙴𝚁 ᴛᴇᴀᴍ
║ 𝙾𝚠𝚗𝚎𝚛:http://wa.me/+918592026326
║ 𝙲𝙾 𝙾𝚆𝙽𝙴𝚁:http://wa.me/+919497513815
║🕯️ ᴘʀᴇғɪx: ${prefix}
...
case 'igpost': 				
		...
		case 'nature3d': 				
				...
				buffer = await getBuffer(anu.result.url)
				denz.sendMessage(from, buffer, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "𝙅𝙊𝘾𝙆𝙀?? 𝙎𝙀𝙍", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('dnsnew.jpg')} } }, caption: 'Nih hasilnya kak...'})
				break
		case 'ocean': 				
				if (args.length < 1) return reply(`where is the text, uncle?`)	
				ct = body.slice(6)
  ...
  break
			...
			if (!isUser) return reply(mess.only.userB)
				...
				break
					...
					mentions(teks, membr, true)
					break
case 'igstory'://𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶 ////𝐁𝐋𝐀𝐂𝐊 𝐓𝐇𝐔𝐍𝐃𝐄𝐑
...
await limitAdd(sender)
break
case 'silveryt'://𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶 ////𝐁𝐋𝐀𝐂𝐊 𝐓𝐇𝐔𝐍𝐃𝐄𝐑
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
lsu1 = body.slice(11)
reply(mess.wait)
...
await limitAdd(sender)
break
case 'goldyt'://𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶 ////𝐁𝐋𝐀𝐂𝐊 𝐓𝐇𝐔𝐍𝐃𝐄𝐑
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
lsu = body.slice(9)
reply(mess.wait)
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
            ...
            break  
				...
				case 'owner':
                 ...
                 denz.sendMessage(from, '*_𝙿𝙼 𝙵𝚁𝙸𝙴𝙽𝙳 𝙰𝙺𝙰𝙼 𝚂𝙰𝚅𝙴 𝙲𝙷𝚈𝚃𝙷𝙾...𝙱𝙾𝚃 𝙻𝙸𝙽𝙺 𝙲𝙷𝙾𝙸𝙺𝙰𝙽𝙳𝙰 :)_*',text, { quoted: mek} )
                 break
                 case 'coowner':
                 denz.sendMessage(from, {displayname: "jeff", vcard: vcard1}, MessageType.contact, { quoted: mek})
                 denz.sendMessage(from, '*_ᴛʜᴀᴛ’s ᴍʏ 𝙲𝙷𝚄𝙽𝙺 𝙹𝙾𝙲𝙺𝙴𝚁 𝚂𝙴𝚁/𝚅𝙸𝙹𝙾 𝚂𝙴𝚁_*',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//akhir kreator
			    ...
			    reply(mess.wait)
			        ...
			        await limitAdd(sender) 
			        break 
			   case 'igstalk'://𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶 ////4 4 4 UYIR
if (isBanned) return reply(mess.only.benned)    
   					if (!isUser) return reply(mess.only.userB)
...
denz.sendMessage(from, buffer, image, {quoted: mek, caption: teks})
break
case 'playstore'://𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶 ////𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶
if (isBanned) return reply(mess.only.benned)    
   					if (!isUser) return reply(mess.only.userB)
...
reply(teks.trim())
break
case 'ytsearch'://𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶 ////𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶
if (isBanned) return reply(mess.only.benned)    
   					if (!isUser) return reply(mess.only.userB)
...
break
case 'wiki'://𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶 ////𝐉𝐎𝐂𝐊𝐄𝐑 𝑆𝐸𝑅 𝑂𝐹𝐶
if (isBanned) return reply(mess.only.benned)    
   					if (!isUser) return reply(mess.only.userB)
reply(mess.wait)
...
case 'husbu':
		...
		setTimeout( () => {
		reply(`*_「 ʟɪɴᴋ ɢʀᴜᴘ ᴅᴇᴛᴇᴄᴛᴇᴅ 」_*\nsᴏʀʀʏ ʏᴏᴜʀ, *${pushname2}* Link etta malare kick chyum no excuse!,\nʏᴏᴜ ᴀʀᴇ ɪᴍᴍᴇᴅɪᴀᴛᴇʟʏ ᴋɪᴄᴋᴇᴅ ғʀᴏᴍ ᴛʜᴇ ɢʀᴏᴜᴘ 𝚋𝚢𝚎 𝚋𝚢𝚎 𝚖𝚎𝚒𝚛 *${groupMetadata.subject}*`)
		}, 0)
		}
   ...
   if (budy.includes("hai99")){
    ...
    if (budy.includes("Sidhu")){
        ...
        denz.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        const d = fs.readFileSync('./sticker/dnsnew.webp');
        denz.sendMessage(from, d, sticker, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "_JOCKER 𝐁𝐎𝐓 𝐔𝐘𝐈??_", 'jpegThumbnail': fs.readFileSync('./sticker/rasheed.webp')}}}})
        }
        if (budy.includes("Sorry")){
		denz.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/sorry.mp3')
